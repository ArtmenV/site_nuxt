  ...
  >
    ...
    <div class="portfolio__collection_items">
      <h4 class="header__table_head">
        <nuxt-link
          class="row-item silent-link highlighted text--bold link-underline"
          :to="`/profile?id=${ownerPortfolio.id}`"
          style="margin-bottom: .4rem"
        >
        {{ ownerPortfolio.username }}
        </nuxt-link>
        Collections
      </h4>
      <TreeFolder 
    ...
    <!-- <div>
      ...
      <div class="portfolio__collection_link--portfolio">
        <nuxt-link
          ...
          :to="`/portfolio/user?user_id=${$route.query.user_id}&path=${parenthPath || 'portfolio'}`"  
          style="margin-bottom: .4rem"
        >
          ...
          >
            ...
            > 
              <nuxt-link
                ...
                :to="`/portfolio/user?user_id=${$route.query.user_id}&path=` + folder.path"
                @click.native="getAllFoldersImages()"
              > 
              <!--  -->
                <svg 
...
export default {
  ...
  methods: {
    ...
    getAllFoldersImages (path) {
      this.isLoading = true

      let rightPath

      if ( !path ) {
        rightPath = encodeURIComponent(this.$route.query.path) !== 'portfolio' ? this.$route.query.path : ''
      } 
      ...
      this.$api.getAllFoldersImage({ user_id: this.$route.query.user_id, path: rightPath })
        .then(({ data: { result } }) => {
          this.folderCollection = result.id || 0
          ...
          this.parenthPath = result.parent_path
          this.ownerPortfolio = result.owner
  ...
  created () {
    if ( this.$route.query.path && this.$route.query.user_id ) {
      this.getAllFoldersImages()
    } 

    if ( !this.$route.query.path && this.$route.query.user_id ) {
      const path = 'portfolio'
      this.getAllFoldersImages(path)
    } 
    // else {
    //   this.isError = true
    // }
